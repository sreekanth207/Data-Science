---
title: "Airline on-time performance analytics"
...
#### Aircraft Engine type vs delays
```{r}
...
There is no strong relationships encountered. 
...
# Based on the co-relations proide in EDA
...
ggplot(AirportDep, aes(md+ma, md1+ma1)) + geom_point() + geom_smooth()
ggplot(AirportDep, aes(md+ma, md1+ma1)) + geom_point(aes(size=DepTimeofday), alpha=1/2) + geom_smooth()
```

 This doesn't say anything. 

We need to agreegate Based on Dest, ArrTime of day and find mean delay at Origin vs Dest and plot
...
### Let's predict on validation data set. 
```{r}
...
It was a great case study both techncally and also to understand different domain. 

### Future imporovements and suggestios

1. We have taken only sample data hence peformance possibly be enhanced by running same analysis Spark. But steps wouldn't change much.
...
4. Airport Cascading delays, precalculated transformed features can be used in model prediction(not used here due to time constranit though answered questions). This will be a very good predictor.
