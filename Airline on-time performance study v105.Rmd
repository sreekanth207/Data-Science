...
We can see that Carrier = WN has managed better on-time perfornance with Arrival (avg ~14min delay)
...
```
Out of above variables, Month adn DepHour seems to be predicting better.
...
We can use alternate methods to choose variables. One of such methods is AIC. This takes combinations of variables and computes statistics. We have run STEP AIC and even with the first model seems to be best. Not showing STEPAIC here to save some real estate. In the actual code file we hae it.
...
## Let's say we want to include UPAPDelay as parameter for predicting Delays. We can write a function.

```{r,message=FALSE}

AascadingAirportDelay <- function(DownstreamAirportval, DownStreamExpHourval ) {
  
   DownstreamAirportDepDelay <- tripsplaneairports %>% 
    filter( Dest == DownstreamAirportval , ArrHour == DownStreamExpHourval ) %>%
    group_by( Dest, ArrHour  ) %>% 
    summarise( DSAPDepDelay = mean(DepDelay)) 
  
  DownstreamAirportArrDelay <- tripsplaneairports %>% 
    filter( Origin == DownstreamAirportval , DepHour == DownStreamExpHourval ) %>%
    group_by( Origin, DepHour  ) %>%
    summarise( DSAPArrDelay = mean(ArrDelay)) 
  ...
  CascadingSummary <- DownStreamUpstreamDelaySummaryfinal %>% 
    group_by(DownStreamAirport, Hour )  %>%
    summarize(DSAPDelay = mean(DSAPDepDelay+DSAPArrDelay), UPAPDelay = mean(UPAPDepDelay+UPAPDepDelay), mean(DSAPDepDelay), mean(DSAPArrDelay) , mean(UPAPDepDelay), mean(UPAPArrDelay) ) 
 
return(if (is_empty(CascadingSummary$UPAPDelay ) ) {0}
else
{
CascadingSummary$UPAPDelay
} 
)
...
```{r,message=FALSE}
AascadingAirportDelay ("ABE", 12 )
AascadingAirportDelay ("ABE", 10 )

```

#### We can use this to create additional feature in the data set. Only in training data set. 
#### In test data set, however this variable will not be available upfront.. i.e This can be used to predict flight delays closer to the time of flying because that is the time the delays in Upstream Airports will be available.

```{r, message=FALSE}
#newtripsplaneairports <- trips %>%  mutate( UpstreamAirportMeanDelay = mapply(AascadingAirportDelay, Dest, ArrHour ))

# Not running this because this goes for loops and takes long time to execute. 
```

###### Now we can use this variable "UpstreamAirportMeanDelay" as a feature to predict flight delays.
...
# First tried loading large files. It's failing due to issues with my spark session.
...
###R-Squared: 74%, RMSE: 41.93. Not a great thing, however these variables are insignificant. 
...
2. We have had 29 + another 10 + varlables to work with. So tried testing each and every variable and it's significance. We could try Principal Component Analysis and then use the principal comonents to predict outcome. 
3. We can also pre-define what is delay, i.e more then certain minutes as delay (e.g: more then 10min as delay) and turn this into Classification problem. And once classified then start predicting the delay minutes using linear regression. To check if there is alternates that can add value. 
4. Airport Cascading delays, precalculated transformed features can be used in model prediction. 
5. Some of the delays such as CarrierDelay etc.. are realtime hence while using this model in production we need to eaither get mean values for a given criteria from historical data and use in prediction or build different model. 
6. Before using this model in production, there is some work to be done listed above. And ofcourse, we need to modularize the code and create functions for all repetative tasks. Also, create functions to answer questions. 
